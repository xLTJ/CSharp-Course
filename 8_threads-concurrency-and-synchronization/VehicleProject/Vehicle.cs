...
public abstract class Vehicle(string model, int year, string color)
{
    ...
    public string Model { get; set; } = !string.IsNullOrWhiteSpace(model)
        ? model
        : throw new ArgumentException("Model cannot empty");

    public string Color { get; set; } = !string.IsNullOrWhiteSpace(color)
        ? color
    ...
    // BUT not anymore cus we need to change it in this program lol
    public abstract int MaxSpeed { get; set; }
    public abstract int MaxEnergy { get; set; }
    public abstract int AccelerateSpeedIncrement { get; set; }
    public abstract int DecelerateSpeedDecrement { get; set; }
    public abstract int CoastingSpeedDecrement { get; set; }
    public abstract int EnergyConsumptionRate { get; set; }
